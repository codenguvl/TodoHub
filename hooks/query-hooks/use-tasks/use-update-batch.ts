...
const useUpdateTasksBatch = () => {
  const queryClient = useQueryClient();

  const { mutate: updateTasksBatch, isLoading: batchUpdating } = useMutation(
    api.tasks.updateBatchTasks,
    {
      // OPTIMISTIC UPDATE
      ...
      onError: (err: AxiosError, newTask, context) => {
        ...
        toast.error({
          message: `Something went wrong while batch updating tasks`,
  ...
  return { updateTasksBatch, batchUpdating };
};

export { useUpdateTasksBatch };
