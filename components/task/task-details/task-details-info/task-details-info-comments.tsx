...
const Comments: React.FC<{ task: TaskType }> = ({ task }) => {
  const scrollRef = useRef(null);
  const [isWritingComment, setIsWritingComment] = useState(false);
  const [isInViewport, ref] = useIsInViewport();
  const { comments, addComment } = useTaskDetails();
...
const CommentPreview: React.FC<{
  comment: GetTaskCommentResponse["comment"];
  user: UserResource | undefined | null;
}> = ({ comment, user }) => {
  const [isEditing, setIsEditing] = useState(false);
  const [isAuthenticated, openAuthModal] = useIsAuthenticated();
  const { updateComment, deleteComment } = useTaskDetails();
