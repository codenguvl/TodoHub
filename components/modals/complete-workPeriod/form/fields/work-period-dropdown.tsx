...
const WorkPeriodDropdownField: React.FC<{
  ...
  return (
    <div className="my-5">
      <Label
        htmlFor="duration"
        text="Chuyển nhiệm vụ mở sang"
        required={false}
      />
      <Controller
        control={control}
        name="moveToWorkPeriodId"
        render={({ field }) => {
          return (
                  ...
                  <SelectViewport className="w-96 min-w-fit rounded-md border border-gray-300 bg-white py-2 shadow-md">
                    <SelectGroup>
                      {workPeriods &&
                        [...workPeriods, backlog]
                          ?.filter(
                            (workPeriod) => workPeriod.status === "PENDING"
                          )
                          ?.map((workPeriod) => (
                            <SelectItem
                              key={workPeriod.id}
      ...
      />
      <Error
        trigger={errors.moveToWorkPeriodId}
        message="Cần chọn một thời gian làm việc mới"
      />
    </div>
  );
};

export { WorkPeriodDropdownField };
