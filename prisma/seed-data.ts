import { type WorkPeriod, type Task, type Comment } from "@prisma/client";
...
export function generateInitialUserWorkPeriods(userId: string): WorkPeriod[] {
  const now = new Date();
...
export function generateInitialUserTasks(userId: string): Task[] {
  const now = new Date();
  const slicedUserId = userId ? userId.slice(5, 12) : "init";
  ...
  return [
    {
      ...
      workPeriodId: "880ececc-f628-4de3-" + slicedUserId,
      isDeleted: false,
      createdAt: now,
      updatedAt: now,
      deletedAt: null,
    ...
    },
    {
      ...
      status: "DONE",
      type: "TASK",
      workPeriodPosition: 2,
      boardPosition: -1,
      reporterId: "user_2PwZmH2xP5aE0svR6hDH4AwDlcu",
      assigneeId: "user_2PvBRngdvenUlFvQNAWbXIvYVy5",
      parentId: null,
      WorkPeriodId: "edd0e2b1-b230-4f02-" + slicedUserId,
      isDeleted: false,
      createdAt: now,
      updatedAt: now,
      deletedAt: null,
